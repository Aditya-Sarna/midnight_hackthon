...
import { proveSessionAuth, type CircledProofChallenge } from "../lib/nyxproof";
import { loadSession } from "../lib/api";

/**
 * External relying-party demo — CircledProof as OTP/2FA replacement.
 * Challenge is public; device proves KYC possession locally; nonce burned once.
 */
export function CircledProofPanel() {
  ...
  async function runAuth() {
    ...
    try {
      ...
      const challenge = (await chRes.json()) as CircledProofChallenge & { error?: string };
      ...
      setOk(false);
      push(e instanceof Error ? e.message : "CircledProof failed");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="circledproof">
      <header className="circledproof__head">
        <p className="theater__eyebrow">CircledProof</p>
